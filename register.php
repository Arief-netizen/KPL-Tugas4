...
if (isset($_POST['regis'])) {
   ...
   if ($password !== $repassword) {
        ...
        $regis = "INSERT INTO user(username, email, password, level, token, status)
            VALUES('".$username."','".$email."','".$password."','".$level."','".$token."','".$status."')";
        $result = mysqli_query($koneksi,$regis);

        $last_id = mysqli_insert_id($koneksi);
        $url = 'http://'.$_SERVER['SERVER_NAME'].'/loginregister/verify.php?id='.$last_id.'&token='.$token;

        $output = '<div>Terimakasih telah melakukan registrasi. Klik tautan berikut untuk memverifikasi akun anda <br>'.$url.'</div>';
        ...
        if ($result == true ) {
           ...
           // $mail->addAttachment('/tmp/image.jpg', 'new.jpg');    // Optional name
            ...
            if(!$mail->send()) {
                ...
                echo "<script>window.alert('Registrasi berhasil, Verifikasi akun anda melalui email')</script>";
            }
